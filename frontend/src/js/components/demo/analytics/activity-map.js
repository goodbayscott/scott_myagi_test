...
// Increase these values to DECREASE how often
// heatmap updates
const DATA_UPDATE_DELAY = 200;
const UPDATE_THRESHOLD = 10;

const INITIAL_HEATMAP_DATA = Im.fromJS({
  max: 8,
  data: [
    ...
    { lat: 25.6586, lng: -80.3568, count: 4 },
...
export default class ActivityMap extends React.Component {
  constructor() {
    super();
    this.state = {
      heatmapData: INITIAL_HEATMAP_DATA
    };
  }
  ...
  componentDidMount() {
    ...
    const map = new L.Map('map-canvas', {
      center: new L.LatLng(32.6586, -90.3568),
      zoom: 4.3,
  ...
  onAnimationFrame(time) {
    if (this.state.lastUpdateTime && time - this.state.lastUpdateTime < DATA_UPDATE_DELAY) {
      return;
    }
    const newHeatmapData = this.state.heatmapData.toJS();
    newHeatmapData.data.forEach(item => {
      if (_.random(-UPDATE_THRESHOLD, 1) > 0) {
        item.count += [-1, 1][_.random(0, 1)];
        if (item.count < 0) item.count = 0;
      }
    });
    this.setState({ heatmapData: Im.fromJS(newHeatmapData), lastUpdateTime: time });
    this.heatmapLayer.setData(newHeatmapData);
  }

  render() {
    // return <div id="map-canvas" style={{ width: 400, height: 400 }} />;
    return (
      <div>
        <div
          style={{
